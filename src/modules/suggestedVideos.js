export default function (playerInstance, options) {
    ...
    playerInstance.generateSuggestedVideoList = () => {
        const configUrl =
            playerInstance.displayOptions.suggestedVideos.configUrl;
        ...
        playerInstance
            .sendRequestAsync(configUrl, false, 5000)
            .then(({ response }) => {
                const config = JSON.parse(response);
                const suggestedVideosGrid =
            ...
            .catch((err) => {
                console.error(
                    '[FP_ERROR] given suggested videos config url is invalid or not found.',
                    err,
                );
            });
    };
    ...
    playerInstance.resetPlayer = (sources, subtitles, configUrl) => {
        const videoDOM = playerInstance.domRef.wrapper.querySelector(
    ...
    playerInstance.createVideoTile = (config) => {
        ...
        playerInstance
            .getImageTwoMostProminentColours(config.thumbnailUrl)
            .then((mostProminentColors) => {
    ...
    playerInstance.resetAds = () => {
        ...
        const checkMainVideoDuration = () => {
            if (
                !isNaN(playerInstance.domRef.player.duration) &&
                playerInstance.domRef.player.duration > 0
            ) {
                playerInstance.toggleLoader(true);
    ...
    playerInstance.removeVideoSourcesListFromDOM = () => {
        const sourcesDOM = playerInstance.domRef.wrapper.getElementsByClassName(
    ...
    playerInstance.removeSubtitlesListFromDOM = () => {
        const tracksDOM = playerInstance.domRef.wrapper.getElementsByClassName(
    ...
    playerInstance.isShowingSuggestedVideos = () => {
        return !!playerInstance.domRef.wrapper.getElementsByClassName(
            'suggested_tile_grid',
        )[0];
    };
}
