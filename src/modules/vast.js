...
export default function (playerInstance, options) {
    ...
    playerInstance.setCTAFromVast = (titleCtaElement, tmpOptions) => {
        if (
            playerInstance.displayOptions.vastOptions.adCTATextVast &&
            titleCtaElement
        ) {
            const mobileText = playerInstance.extractNodeDataByTagName(
                titleCtaElement,
                'MobileText',
            );
            const desktopText = playerInstance.extractNodeDataByTagName(
                titleCtaElement,
                'PCText',
            );
            const link =
                playerInstance.extractNodeDataByTagName(
                    titleCtaElement,
                    'DisplayUrl',
                ) ||
                playerInstance.extractNodeDataByTagName(
            ...
            const tracking = playerInstance.extractNodeDataByTagName(
                titleCtaElement,
                'Tracking',
            );
    ...
    playerInstance.getVastAdTagUriFromWrapper = (xmlResponse) => {
        ...
        if (typeof wrapper !== 'undefined' && wrapper.length) {
            const vastAdTagURI =
                wrapper[0].getElementsByTagName('VASTAdTagURI');
    ...
    playerInstance.getClickThroughUrlFromNonLinear = (nonLinear) => {
        ...
        if (nonLinears.length) {
            //There should be exactly 1 node
            const nonLinearClickThrough = nonLinear.getElementsByTagName(
                'NonLinearClickThrough',
            );
            if (nonLinearClickThrough.length) {
                result = playerInstance.extractNodeData(
                    nonLinearClickThrough[0],
                );
            }
        }
    ...
    playerInstance.getDurationFromNonLinear = (tag) => {
        ...
        if (
            ...
            result = playerInstance.convertTimeStringToSeconds(
    ...
    playerInstance.getIconClickThroughFromLinear = (linear) => {
        const iconClickThrough =
            linear.getElementsByTagName('IconClickThrough');
    ...
    playerInstance.getStaticResourceFromNonLinear = (linear) => {
        ...
        for (let i = 0; i < staticResources.length; i++) {
            if (!staticResources[i].getAttribute('type')) {
                fallbackStaticResource = playerInstance.extractNodeData(
                    staticResources[i],
                );
            }
            ...
            if (
                ...
                playerInstance.displayOptions.staticResource
            ) {
    ...
    playerInstance.registerTrackingEvents = (creativeLinear, tmpOptions) => {
        const trackingEvents =
            playerInstance.getTrackingFromLinear(creativeLinear);
        let eventType = '';
        ...
        for (let i = 0; i < trackingEvents.length; i++) {
            ...
            switch (eventType) {
                ...
                case 'complete':
                    ...
                    if (
                        typeof tmpOptions.stopTracking[eventType] ===
                        'undefined'
                    ) {
                        tmpOptions.stopTracking[eventType] = [];
                    }
                    tmpOptions.tracking[eventType].push(
                        trackingEvents[i].textContent.trim(),
                    );
                ...
                case 'progress':
                    ...
                    oneEventOffset = playerInstance.convertTimeStringToSeconds(
                        trackingEvents[i].getAttribute('offset'),
                    );

                    if (
                        typeof tmpOptions.tracking[eventType][
                            oneEventOffset
                        ] === 'undefined'
                    ) {
                    ...
                    tmpOptions.tracking[eventType][
                        oneEventOffset
                    ].elements.push(trackingEvents[i].textContent.trim());
    ...
    playerInstance.registerViewableImpressionEvents = (
        viewableImpressionTags,
        tmpOptions,
    ) => {
        ...
        for (let i = 0; i < viewableImpressionTags.length; i++) {
            const viewableImpressionEvent = playerInstance.extractNodeData(
                viewableImpressionTags[i],
            );
    ...
    playerInstance.registerImpressionEvents = (impressionTags, tmpOptions) => {
        ...
        for (let i = 0; i < impressionTags.length; i++) {
            const impressionEvent = playerInstance.extractNodeData(
                impressionTags[i],
            );
    ...
    playerInstance.announceError = (code) => {
        if (
            ...
            !playerInstance.vastOptions.errorUrl
        ) {
            return;
        }

        const parsedCode = typeof code !== 'undefined' ? parseInt(code) : 900;
        const errorUrl = playerInstance.vastOptions.errorUrl.replace(
            '[ERRORCODE]',
            parsedCode,
        );
    ...
    playerInstance.getClickTrackingEvents = (linear) => {
        ...
        for (let i = 0; i < clickTracking.length; i++) {
            const clickTrackingEvent = playerInstance.extractNodeData(
                clickTracking[i],
            );
    ...
    playerInstance.getNonLinearClickTrackingEvents = (nonLinear) => {
        ...
        const clickTracking = nonLinear.getElementsByTagName(
        ...
        for (let i = 0; i < clickTracking.length; i++) {
            const NonLinearClickTracking = playerInstance.extractNodeData(
                clickTracking[i],
            );
    ...
    playerInstance.recalculateAdDimensions = () => {
        ...
        const divClickThrough = playerInstance.domRef.wrapper.querySelector(
        ...
        const requestFullscreenFunctionNames =
            playerInstance.checkFullscreenSupport();
        const fullscreenButton = playerInstance.domRef.wrapper.querySelector(
            '.fluid_control_fullscreen',
        );
        const menuOptionFullscreen =
        ...
        if (requestFullscreenFunctionNames) {
            ...
            if (
                document[requestFullscreenFunctionNames.isFullscreen] === null
            ) {
                // Exit fullscreen
                playerInstance.fullscreenOff(
                    fullscreenButton,
                    menuOptionFullscreen,
                );
            } else {
                // Go fullscreen
                playerInstance.fullscreenOn(
                    fullscreenButton,
                    menuOptionFullscreen,
                );
            }
    ...
    playerInstance.prepareVast = (roll) => {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            playerInstance.processVastWithRetries(
                playerInstance.rollsById[rollListId],
            );
        }
    };
    ...
    function processAdCreatives(ad) {
        ...
        const creativeElements = Array.from(
            adElement.getElementsByTagName('Creative'),
        );
                ...
                try {
                    if (ad.adType === 'linear') {
                        const linearCreatives =
                            creativeElement.getElementsByTagName('Linear');
                        const creativeLinear = linearCreatives[0];

                        //Extract the Ad data if it is actually the Ad (!wrapper)
                        if (
                            !playerInstance.hasVastAdTagUri(adElement) &&
                            playerInstance.hasInLine(adElement)
                        ) {
                            //Set initial values
                            ad.adFinished = false;
                            ad.vpaid = false;
                            ...
                            ad.skipoffset =
                                playerInstance.convertTimeStringToSeconds(
                                    creativeLinear.getAttribute('skipoffset'),
                                );
                            ad.clickthroughUrl =
                                playerInstance.getClickThroughUrlFromLinear(
                                    creativeLinear,
                                );
                            ad.duration =
                                playerInstance.getDurationFromLinear(
                                    creativeLinear,
                                );
                            ad.mediaFileList =
                                playerInstance.getMediaFileListFromLinear(
                                    creativeLinear,
                                );
                            ad.adParameters =
                                playerInstance.getAdParametersFromLinear(
                                    creativeLinear,
                                );
                            ad.iconClick =
                                ad.iconClick ||
                                playerInstance.getIconClickThroughFromLinear(
                                    creativeLinear,
                                );
                    ...
                    if (ad.adType === 'nonLinear') {
                        const nonLinearCreatives =
                            creativeElement.getElementsByTagName(
                                'NonLinearAds',
                            );
                        ...
                        if (
                            ...
                            ad.clickthroughUrl =
                                playerInstance.getClickThroughUrlFromNonLinear(
                                    creativeNonLinear,
                                );
                            ad.duration =
                                playerInstance.getDurationFromNonLinear(
                                    creativeNonLinear,
                                ); // VAST version < 4.0
                            ad.dimension =
                                playerInstance.getDimensionFromNonLinear(
                                    creativeNonLinear,
                                ); // VAST version < 4.0
                            ad.staticResource =
                                playerInstance.getStaticResourceFromNonLinear(
                                    creativeNonLinear,
                                );
                            ad.creativeType =
                                playerInstance.getCreativeTypeFromStaticResources(
                                    creativeNonLinear,
                                );
                            ad.adParameters =
                                playerInstance.getAdParametersFromLinear(
                    ...
                    if (
                        ...
                        !(
                            creativeElement.firstElementChild.tagName ===
                                'Linear' ||
                            creativeElement.firstElementChild.tagName ===
                                'NonLinearAds'
                        )
    ...
    playerInstance.processVastWithRetries = (vastObj) => {
        ...
        playerInstance.domRef.player.addEventListener(
            'adId_' + rollListId,
            playerInstance[vastObj.roll],
        );

        const handleVastResult = function (pass, adOptionsList) {
            if (
                pass &&
                Array.isArray(adOptionsList) &&
                !playerInstance.displayOptions.vastOptions.allowVPAID &&
                adOptionsList.some((adOptions) => adOptions.vpaid)
            ) {
                adOptionsList = adOptionsList.filter(
                    (adOptions) => adOptions.vpaid !== true,
                );
                playerInstance.announceLocalError(
                    '103',
            ...
            if (pass && Array.isArray(adOptionsList) && adOptionsList.length) {
                ...
                adOptionsList.forEach((tmpOptions, index) => {
                    ...
                    tmpOptions.adType = tmpOptions.adType
                        ? tmpOptions.adType
                        : 'unknown';
                    playerInstance.adPool[rollListId].push(
                ...
                if (
                    vastObj.hasOwnProperty('fallbackVastTags') &&
                    vastObj.fallbackVastTags.length > 0
                ) {
                    vastTag = vastObj.fallbackVastTags.shift();
                    playerInstance.processUrl(
                        vastTag,
                        handleVastResult,
                        rollListId,
                    );
     ...
     */
    async function resolveAdTreeRequests(
        url,
        maxDepth,
        baseNode = {},
        currentDepth = 0,
        ...
        if (playerInstance.displayOptions.vastOptions.vastStatic) {
            ...
            const adElements = Array.from(
                responseXML.getElementsByTagName('Ad'),
            );
        ...
        const { responseXML } = await playerInstance.sendRequestAsync(
            url,
            playerInstance.displayOptions.vastOptions.vastWithCredentials,
        ...
        for (const adElement of adElements) {
            const vastAdTagUri =
                playerInstance.getVastAdTagUriFromWrapper(adElement);
            ...
            if (
                ...
                const [wrapperElement] =
                    adElement.getElementsByTagName('Wrapper');
                const disableAdditionalWrappers =
                    wrapperElement.attributes.followAdditionalWrappers &&
                    ['false', '0'].includes(
                        wrapperElement.attributes.followAdditionalWrappers
                            .value,
                    ); // See VAST Wrapper spec
                const allowMultipleAds =
                    wrapperElement.attributes.allowMultipleAds &&
                    ['true', '1'].includes(
                        wrapperElement.attributes.allowMultipleAds.value,
                    ); // See VAST Wrapper spec
                const fallbackOnNoAd =
                    wrapperElement.attributes.fallbackOnNoAd &&
                    ['true', '1'].includes(
                        wrapperElement.attributes.fallbackOnNoAd.value,
                    );

                try {
                    const wrapperResponse = await resolveAdTreeRequests(
                        vastAdTagUri,
                        maxDepth,
                        { tagType: 'wrapper', ...adNode, fallbackOnNoAd },
                        currentDepth + 1,
                    ...
                    if (!allowMultipleAds || isAdPod) {
                        wrapperResponse.children = getFirstStandAloneAd(
                    ...
                    playerInstance.debugMessage(
                        `Error when loading Wrapper, will trigger fallback if available`,
                        e,
                    );
                }
    ...
    function flattenAdTree(root, ads = [], wrappers = []) {
        ...
        if (Array.isArray(root.children) && root.children.length) {
            root.children.forEach((child) =>
                flattenAdTree(child, ads, currentWrappers),
            );
        }
    ...
    function registerAdProperties(rawAd, options) {
        ...
        ad.adType =
            (ad.data.getElementsByTagName('Linear').length && 'linear') ||
            (ad.data.getElementsByTagName('NonLinearAds').length &&
                'nonLinear') ||
            'unknown';
        ...
        [...(ad.wrappers || []), ad.data]
            ...
            .forEach((dataSource) => {
                ...
                const impression =
                    dataSource.getElementsByTagName('Impression');
                if (impression !== null) {
                ...
                const viewableImpression =
                    dataSource.getElementsByTagName('Viewable');
                ...
                const clickTracks =
                    ad.adType === 'linear'
                        ...
                        : playerInstance.getNonLinearClickTrackingEvents(
                              dataSource,
                          );
                playerInstance.registerClickTracking(clickTracks, ad);
            });

        ad.sequence = ad.data.attributes.sequence
            ? Number(ad.data.attributes.sequence.value)
            : null;
        ad.played = false;
    ...
    function getPlayableAds(ads, maxDuration, maxQuantity, forceStandAloneAd) {
        const { adPod } = ads
            ...
            .reduce(
                (playableAds, ad) => {
                    if (
                        playableAds.adPod.length < maxQuantity &&
                        playableAds.totalDuration + ad.duration <= maxDuration
                    ) {
        ...
        const adBuffet = ads.filter(
            (ad) => !Boolean(ad.sequence) && ad.duration < maxDuration,
        );

        const isValidAdPodFormats = adPod
            .map((ad) => ad.adType)
            .slice(0, -1)
            .every((adType) => adType === 'linear');
        ...
        if (adPod.length > 0 && !forceStandAloneAd && isValidAdPodFormats) {
            ...
            playerInstance.debugMessage(
                'Trying to play single ad, adBuffet:',
                adBuffet,
            );
        ...
        resolveAdTreeRequests(
            ...
            .then((result) => {
                try {
                    ...
                    const triggerFallbackOnNoAd = result.children.some(
                        (ad) =>
                            ad.tagType === 'wrapper' &&
                            ad.fallbackOnNoAd &&
                            (!/"tagType":"ad"/.test(JSON.stringify(ad)) ||
                                ad.httpError),
                    );

                    if (triggerFallbackOnNoAd) {
                        playerInstance.debugMessage(
                            'Error on VAST Wrapper, triggering fallbackOnNoAd. Ad tree:',
                            result,
                        );
                    }

                    result = flattenAdTree(result).map((ad) =>
                        processAdCreatives(
                            registerAdProperties(ad, tmpOptions),
                        ),
                    );

                    const playableAds = getPlayableAds(
                        result,
                        playerInstance.rollsById[rollListId].maxTotalDuration ||
                            Number.MAX_SAFE_INTEGER,
                        playerInstance.rollsById[rollListId].maxTotalQuantity ||
                            Number.MAX_SAFE_INTEGER,
                        triggerFallbackOnNoAd,
                    );

                    playableAds && playableAds.length
                        ? callback(true, playableAds)
                        : callback(false);
                } catch (error) {
                    callback(false);
                }
    ...
    playerInstance.setVastList = () => {
        ...
        const validateRequiredParams = function (item) {
            ...
            if (playerInstance.availableRolls.indexOf(item.roll) === -1) {
                playerInstance.announceLocalError(
                    102,
                    'Only ' +
                        playerInstance.availableRolls.join(',') +
                        ' rolls are supported.',
                );
                hasError = true;
            }
            ...
            if (
                item.size &&
                playerInstance.supportedNonLinearAd.indexOf(item.size) === -1
            ) {
                playerInstance.announceLocalError(
                    102,
                    'Only ' +
                        playerInstance.supportedNonLinearAd.join(',') +
                        ' size are supported.',
                );
                hasError = true;
            }
        ...
        if (
            playerInstance.displayOptions.vastOptions.hasOwnProperty('adList')
        ) {
            for (let key in playerInstance.displayOptions.vastOptions.adList) {
                let rollItem =
                    playerInstance.displayOptions.vastOptions.adList[key];

                if (validateRequiredParams(rollItem)) {
                    playerInstance.announceLocalError(
                        102,
                        'Wrong adList parameters.',
                    );
                    continue;
                }
                const id = 'ID' + idPart;
        ...
        Object.keys(rolls).map(function (e) {
            switch (rolls[e].roll.toLowerCase()) {
                ...
                default:
                    console.error(
                        `${rolls[e].roll.toLowerCase()} is not a recognized roll`,
                    );
                    break;
            }
        });
    ...
    playerInstance.vastLogoBehaviour = (vastPlaying) => {
        if (!playerInstance.displayOptions.layoutControls.logo.showOverAds) {
            const logoHolder =
                playerInstance.domRef.wrapper.querySelector('.logo_holder');
